...
namespace SmartIOT.Connector.RestApi.Controllers.V1
{
	...
	public class ConnectorController : ControllerBase
	{
		...
		[ProducesResponseType(StatusCodes.Status404NotFound)]
		[ProducesResponseType(StatusCodes.Status200OK, Type = typeof(Model.Connector))]
		public IActionResult UpdateConnector(int index, [FromBody] string connectionString)
		{
			if (_connectorsService.ReplaceConnector(index, connectionString))
		...
		public IActionResult AddConnector([FromBody] string connectionString)
		{
			var connector = _connectorsService.AddConnector(connectionString);
			if (connector != null)
		...
		public IActionResult RemoveConnector(int index)
		{
			if (_smartIotConnector.RemoveConnector(index))
				return Ok();
